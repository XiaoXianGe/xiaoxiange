...
-(void)showViews:(BOOL)boolens withWidth:(CGFloat)width WithHeight:(CGFloat)height{
    ...
    if (boolens==YES) {
        //关闭按钮
        UIButton *btn=[[UIButton alloc]initWithFrame:CGRectMake(230, 10, 40, 30)];
...
-(void)layoutSubviews{
    ...
    self.contentView.frame = CGRectMake(0,50,self.contentView.frame.size.width,self.contentView.frame.size.height + 200);
